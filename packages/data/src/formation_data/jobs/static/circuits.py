"""Static job — seed the Circuit table from the hand-curated list.

Cadence: rare. Re-run only when the FIA calendar gains/loses a circuit, when sm_zones
are confirmed, or when track measurements change.

Data is hand written. sm_zones yet to be confirmed for future races (currently reflects DRS zones)

jolpica_id and lat/lon were verified against the live Jolpica 2026 schedule
(GET /ergast/f1/2026.json — Circuit.circuitId and Circuit.Location).

fastf1_location is FastF1's get_event_schedule().Location, the stable cross-source
key used to find a circuit's rounds (Country isn't unique, EventName is season-
unstable). Values are the current (2026) Location; a few venues have been renamed
across seasons (Miami/Miami Gardens, Monaco/Monte Carlo, Yas Island/Yas Marina),
and those historical variants are handled in fastf1_client._LOCATION_ALIASES.

layout_since_year is the first season of the *current* track configuration, set
only where a modern-era reprofiling makes older laps non-comparable (so the lap
record isn't taken from a superseded layout). Omitted = layout stable across the
available data. VERIFY these years against official sources before relying on them.
"""

from __future__ import annotations

import logging

from sqlalchemy import Connection
from formation_data.domain import Circuit

logger = logging.getLogger(__name__)


def run(conn: Connection) -> None:
    from formation_data import repositories, schema

    # track_outline is generated by jobs.pre_season.track_maps, not part of this
    # hand-authored seed — preserve it on conflict so re-seeding doesn't wipe it.
    repositories.upsert(
        conn,
        table=schema.circuits,
        items=CIRCUITS,
        conflict_cols=["circuit_id"],
        update_exclude={"track_outline"},
    )
    logger.info("static.circuits.run")


CIRCUITS = [
    Circuit(
        circuit_id="melbourne",
        fastf1_location="Melbourne",
        country="Australia",
        track_length_km=5.278,
        num_corners=14,
        num_laps=58,
        sm_zones=5,
        jolpica_id="albert_park",
        lat=-37.8497,
        lon=144.968,
        layout_since_year=2022,  # Albert Park reprofiled (several corners) for 2022
    ),
    Circuit(
        circuit_id="shanghai",
        fastf1_location="Shanghai",
        country="China",
        track_length_km=5.451,
        num_corners=16,
        num_laps=56,
        sm_zones=4,
        jolpica_id="shanghai",
        lat=31.3389,
        lon=121.22,
    ),
    Circuit(
        circuit_id="suzuka",
        fastf1_location="Suzuka",
        country="Japan",
        track_length_km=5.807,
        num_corners=18,
        num_laps=53,
        sm_zones=2,
        jolpica_id="suzuka",
        lat=34.8431,
        lon=136.541,
    ),
    Circuit(
        circuit_id="miami",
        fastf1_location="Miami Gardens",
        country="United States",
        track_length_km=5.412,
        num_corners=19,
        num_laps=57,
        sm_zones=3,
        jolpica_id="miami",
        lat=25.9581,
        lon=-80.2389,
    ),
    Circuit(
        circuit_id="montreal",
        fastf1_location="Montréal",
        country="Canada",
        track_length_km=4.361,
        num_corners=14,
        num_laps=70,
        sm_zones=3,
        jolpica_id="villeneuve",
        lat=45.5,
        lon=-73.5228,
    ),
    Circuit(
        circuit_id="monaco",
        fastf1_location="Monte Carlo",
        country="Monaco",
        track_length_km=3.337,
        num_corners=19,
        num_laps=78,
        sm_zones=0,
        jolpica_id="monaco",
        lat=43.7347,
        lon=7.42056,
    ),
    Circuit(
        circuit_id="barcelona",
        fastf1_location="Barcelona",
        country="Spain",
        track_length_km=4.657,
        num_corners=16,
        num_laps=66,
        sm_zones=2,
        jolpica_id="catalunya",
        lat=41.57,
        lon=2.26111,
        layout_since_year=2023,  # final chicane removed for 2023
    ),
    Circuit(
        circuit_id="red_bull_ring",
        fastf1_location="Spielberg",
        country="Austria",
        track_length_km=4.318,
        num_corners=10,
        num_laps=71,
        sm_zones=3,
        jolpica_id="red_bull_ring",
        lat=47.2197,
        lon=14.7647,
    ),
    Circuit(
        circuit_id="silverstone",
        fastf1_location="Silverstone",
        country="United Kingdom",
        track_length_km=5.891,
        num_corners=18,
        num_laps=52,
        sm_zones=2,
        jolpica_id="silverstone",
        lat=52.0786,
        lon=-1.01694,
        layout_since_year=2010,  # Arena layout (new infield section) from 2010
    ),
    Circuit(
        circuit_id="spa",
        fastf1_location="Spa-Francorchamps",
        country="Belgium",
        track_length_km=7.004,
        num_corners=19,
        num_laps=44,
        sm_zones=2,
        jolpica_id="spa",
        lat=50.4372,
        lon=5.97139,
    ),
    Circuit(
        circuit_id="hungaroring",
        fastf1_location="Budapest",
        country="Hungary",
        track_length_km=4.381,
        num_corners=14,
        num_laps=70,
        sm_zones=2,
        jolpica_id="hungaroring",
        lat=47.5789,
        lon=19.2486,
    ),
    Circuit(
        circuit_id="zandvoort",
        fastf1_location="Zandvoort",
        country="Netherlands",
        track_length_km=4.259,
        num_corners=14,
        num_laps=72,
        sm_zones=2,
        jolpica_id="zandvoort",
        lat=52.3888,
        lon=4.54092,
    ),
    Circuit(
        circuit_id="monza",
        fastf1_location="Monza",
        country="Italy",
        track_length_km=5.793,
        num_corners=11,
        num_laps=53,
        sm_zones=2,
        jolpica_id="monza",
        lat=45.6156,
        lon=9.28111,
    ),
    Circuit(
        circuit_id="madrid",
        fastf1_location="Madrid",
        country="Spain",
        track_length_km=5.476,
        num_corners=22,
        num_laps=57,
        sm_zones=3,
        jolpica_id="madring",
        lat=40.46528,
        lon=-3.61528,
    ),
    Circuit(
        circuit_id="baku",
        fastf1_location="Baku",
        country="Azerbaijan",
        track_length_km=6.003,
        num_corners=20,
        num_laps=51,
        sm_zones=2,
        jolpica_id="baku",
        lat=40.3725,
        lon=49.8533,
    ),
    Circuit(
        circuit_id="singapore",
        fastf1_location="Marina Bay",
        country="Singapore",
        track_length_km=4.940,
        num_corners=19,
        num_laps=62,
        sm_zones=3,
        jolpica_id="marina_bay",
        lat=1.2914,
        lon=103.864,
        layout_since_year=2023,  # reconfigured (grandstand section removed) for 2023
    ),
    Circuit(
        circuit_id="austin",
        fastf1_location="Austin",
        country="United States",
        track_length_km=5.513,
        num_corners=20,
        num_laps=56,
        sm_zones=2,
        jolpica_id="americas",
        lat=30.1328,
        lon=-97.6411,
    ),
    Circuit(
        circuit_id="mexico_city",
        fastf1_location="Mexico City",
        country="Mexico",
        track_length_km=4.304,
        num_corners=17,
        num_laps=71,
        sm_zones=3,
        jolpica_id="rodriguez",
        lat=19.4042,
        lon=-99.0907,
    ),
    Circuit(
        circuit_id="sao_paulo",
        fastf1_location="São Paulo",
        country="Brazil",
        track_length_km=4.309,
        num_corners=15,
        num_laps=71,
        sm_zones=2,
        jolpica_id="interlagos",
        lat=-23.7036,
        lon=-46.6997,
    ),
    Circuit(
        circuit_id="las_vegas",
        fastf1_location="Las Vegas",
        country="United States",
        track_length_km=6.201,
        num_corners=17,
        num_laps=50,
        sm_zones=2,
        jolpica_id="vegas",
        lat=36.1147,
        lon=-115.173,
    ),
    Circuit(
        circuit_id="lusail",
        fastf1_location="Lusail",
        country="Qatar",
        track_length_km=5.419,
        num_corners=16,
        num_laps=57,
        sm_zones=2,
        jolpica_id="losail",
        lat=25.49,
        lon=51.4542,
    ),
    Circuit(
        circuit_id="abu_dhabi",
        fastf1_location="Yas Marina",
        country="United Arab Emirates",
        track_length_km=5.281,
        num_corners=16,
        num_laps=58,
        sm_zones=2,
        jolpica_id="yas_marina",
        lat=24.4672,
        lon=54.6031,
        layout_since_year=2021,  # extensively reprofiled for 2021
    ),
]
